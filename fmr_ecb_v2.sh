...
echo $fmr_status | jq -r '
  ...
  def header: if .Datasets[0].Dataflow then "Dataflow" else "DSD" end;
  def value: (.Datasets[0].Dataflow // .Datasets[0].DSD | split("=")[1]);
  def dash_line: value | length | [range(.) | "-"] | join("");

  [header, "KeysCount", "ObsCount", "GroupsCount", "ErrorCount"],
  [dash_line, "---------", "--------", "-----------", "----------"],
  [value, (.Datasets[0].KeysCount|tostring), (.Datasets[0].ObsCount|tostring),
